...
namespace Incontra.Packer.Api.Controllers
{
    ...
    public class PackController : ApiController
    {
        ...
        private List<InputContainer> GetJsonContainers(List<Container> containers, Models.PackRequest request)
        {
            ...
            var packRequest = new Data.Model.PackRequest
            {
                UserID = 1,
                RequestDate = DateTime.Now,
                CalculationTime = _executionTime
            };
            //packRequest = _packerService.PackRequestInsert(packRequest);
            _spaceTotal = 0;
            _spaceUsed = 0;
            foreach (var container in containers)
            {
                ...
                var containerVisualizerFolder = GetDateFolderName();
                var containerVisualizerFile = String.Format("container.3d.{0}.html", containerGuid);
                var containerVisualizerContent = GetContainerVisualizerContent(container);
                SaveContainerVisualizerFile(container, containerVisualizerContent, containerVisualizerFolder, containerVisualizerFile);

                var visualizer3D = string.Format(@"
                    <iframe id=""3dp"" width=""300"" height=""300"" style=""border: none;"" src=""{0}://{1}:{2}/Content/Visualizers/{3}/{4}""></iframe>"
                    , Request.RequestUri.Scheme
                    , Request.RequestUri.Host
                    , Request.RequestUri.Port
                    , containerVisualizerFolder
                    , containerVisualizerFile);
        ...
        private string GetContainerVisualizerContent(Container container)
        {              
            var containerVisualizerContent = String.Format(@"
            ...
            foreach (var box in container.Boxes)
            {
                containerVisualizerContent += String.Format(@"
            ...
            return containerVisualizerContent;
        }

        private void SaveContainerVisualizerFile(Container container, string containerVisualizerContent, string containerVisualizerFolder, string containerVisualizerFile)
        {
            ...
            containerVisualizerContent = String.Format(container3dTemplate, containerVisualizerContent, cameraDistance);
            if (!Directory.Exists(Path.Combine(path, containerVisualizerFolder)))
                Directory.CreateDirectory(Path.Combine(path, containerVisualizerFolder));
            File.WriteAllText(Path.Combine(Path.Combine(path, containerVisualizerFolder), containerVisualizerFile), containerVisualizerContent);
        }
